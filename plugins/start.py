    ...
    if left > 0:
        ...
        if value < file.file_size:
            if STRING:
                if buy_date == None:
                    await message.reply_text(
                        f"You Can't Upload More Then {humanbytes(limit)} Used Daly Limit {humanbytes(used)} ",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Upgrade 💰", callback_data="upgrade")]]))
                    return
            ...
            total_size(int(botid), prsize, file.file_size)
            await message.reply_text(
                f"""**🗂️ What Do You Want Me To Do With This File ?**\n\n**◈ File Name :-** {filename}\n**◈ File Size :-** {filesize}\n**◈ Dc ID :-** {dcid}""",
                reply_to_message_id=message.id,
                reply_markup=InlineKeyboardMarkup(
